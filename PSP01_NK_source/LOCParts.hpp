...
using namespace std;
/*number of Items::Items in each part*/
class Parts
{
  public:
	std::string partName;
	std::map< Items::Items* , int >items; //Items::Items to be screen and counting
	char beginChar;
	char endChar;
	...
	std::list<Items::Items* > call_stack; //storing call stack
	std::string input_line;
	//Items::Items* last_item; //temp storing the itemss
	int number_itemfound;
	Items::Items* item_to_look_for;
	int pos;
	...
	void detailsPrint(); //Print out all the Items::Items
	int getTotal();

	void countValiditemsInPart(std::string string_input ,int line_number); //Plus the number of Items::Items (if found)
	int countItem( int start_position);//Check all Items::Items in the config
	...
	Items::Items* fetchFirstToken(int line_size,int line_start); //fetch the first token of the line
